...
const Header = () => {
  const {ToggleMobileView,activeMenu, setactiveMenu,setveiwcart,ToggleOverflow,cartItems,currentLocation} = useContext(AppContext)
  return (
    <>
  ...
  {NavMenu.map((each,index) => <a href ={`${each.Link}`} key = {index} className= {currentLocation === each.Link ? 'text-3xl font-bold hover:text-white  cursor-pointer text-purple-600' : 'text-gray-400 text-3xl font-bold hover:text-white cursor-pointer'} onClick={() => setactiveMenu(index)}>{each.description}</a>)}
</ul>
          <Link to={'/veiwcart'} className="flex   text-white my-auto cursor-pointer relative ">
