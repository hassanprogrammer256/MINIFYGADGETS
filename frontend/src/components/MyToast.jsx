...
const MyToast = () => {
    const { istoast, setistoast, setveiwcart, ToggleOverflow } = useContext(AppContext);
    ...
    return (
        ...
        >
            <div className="md:text-4xl text-end flex justify-between items-center py-2 text-white font-normal">
                ...
                <span className='text-black hover:text-white font-bold md:text-3xl text-2xl cursor-pointer' onClick={() => { setistoast(false); setveiwcart(true); ToggleOverflow(); }}>
                    View Cart
                </span>
                <X size={20} className='cursor-pointer hover:text-black font-black mt-2' onClick={() => { setistoast(false); }} />
            </div>
